...
typedef unsigned char BYTE;
 ...
 inline bool FHexCharValid(char c)
{
	if (c >= '0' && c <= '9' ||
		c >= 'A' && c <= 'F' ||
		c >= 'a' && c <= 'f' ||
 ...
 inline bool FHexDecoder(char* Dec, char* Src)
{
	char HighC, LowC;
...
inline bool AobScan::__SundayHexInit__(char* Sub, unsigned long*p, char* HexSub, unsigned long dwSubLen)
{
...
inline ULONG64 AobScan::__SundayHex__(char* Src, unsigned long dwSrcLen, char* Sub, DWORD* p, char* HexSub, DWORD dwSubLen)
{
	//开始配对字符串
...
inline ULONG64 AobScan::__SundayHexV__(char* Src, unsigned long dwSrcLen, char* Sub, DWORD* p, char* HexSub, DWORD dwSubLen, int v)
{
	//开始配对字符串
	//j为 Sub位置指标， k为 当前匹配位置
	ULONG64 j, k;
...
inline int AobScan::SundayHex(char* Src, unsigned long dwSrcLen, char* Sub)
{
	...
	DWORD* p = new DWORD[0x100];        //P表 标志距离
	int i = -1;
...
inline vector<ULONG64> AobScan::SundayHexV(char* Src, unsigned long dwSrcLen, char* Sub)
{
	vector<ULONG64> v;
	DWORD dwSubLen = strlen(Sub);
...
inline std::vector <DWORD> AobScan::FindSigX32(DWORD dwPid, const char* Value, ULONG64 Start, ULONG64 End)
{
